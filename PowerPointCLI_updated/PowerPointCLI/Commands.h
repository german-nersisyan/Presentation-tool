...
public:
	AddShapeCommand(std::shared_ptr<IShape> s, Model& m, IView& v)
		: shape(std::move(s)), model(m), view(v) {}
...
class ShowSlidesCommand : public ICommand {
	Model& model;
	IView& view;
	int index; // -1 = all
public:
	ShowSlidesCommand(Model& m, IView& v, int idx = -1);
...
class SelectSlideCommand : public ICommand {
	Model& model;
	IView& view;
	int index;
	int previousSlide = -1;
public:
	SelectSlideCommand(Model& m, IView& v, int idx);
